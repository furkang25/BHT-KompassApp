...
public class ScheduleTuesdayActivity extends AppCompatActivity {
    ...
    private void startScheduleAlert (String alert, String message, int scheduleNumber) {
        AlertDialog alertDialog = new AlertDialog.Builder(ScheduleTuesdayActivity.this).create();
    ...
    public void deleteSchedule (int scheduleNumber) throws ParseException {
        switch (scheduleNumber) {
            case 1:
                deleteScheduleDatabase("Dienstag", "8:00 - 9:30", ParseUser.getCurrentUser().getUsername());
                break;
            case 2:
                deleteScheduleDatabase("Dienstag", "10:00 - 11:30", ParseUser.getCurrentUser().getUsername());
                break;
            case 3:
                deleteScheduleDatabase("Dienstag", "12:15 - 13:45", ParseUser.getCurrentUser().getUsername());
                break;
            case 4:
                deleteScheduleDatabase("Dienstag", "14:15 - 15:45", ParseUser.getCurrentUser().getUsername());
                break;
            case 5:
                deleteScheduleDatabase("Dienstag", "16:00 - 17:30", ParseUser.getCurrentUser().getUsername());
                break;
            case 6:
                deleteScheduleDatabase("Dienstag", "17:45 - 19:15", ParseUser.getCurrentUser().getUsername());
                break;
            case 7:
                deleteScheduleDatabase("Dienstag", "19:30 - 21:00", ParseUser.getCurrentUser().getUsername());
                break;
        }
    }
    ...
    private void chooseAddOption() {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(ScheduleTuesdayActivity.this);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent resultData) {
        ...
        if (requestCode == CHOOSE_PDF_FROM_DEVICE && resultCode == Activity.RESULT_OK) {
            ...
            if (resultData != null) {
                ...
                try {
                    InputStream inputStream = ScheduleTuesdayActivity.this.getContentResolver().openInputStream(selectedPdfFromStorage);
    ...
    public void createObject(String module, String prof, String room, String time) {
        ...
        entity.put("day", "Dienstag");
        entity.put("time", time);
        entity.put("room", room);
        entity.put("module", module);
        entity.put("dayTimeUser", "Dienstag" + time + currentUser.getUsername());
    ...
    public void loadData () {
        ...
        query.whereEqualTo("day", "Dienstag");
        query.findInBackground((objects, e) -> {
            if (e == null) {
                for (int i = 0; i < objects.size(); i++) {
